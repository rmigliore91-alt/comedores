console.log("Comedores Component Booting (Cloud Mode)...");
...
// ═══════════════════════════════════════════════════════════════════════════════
// STREAMLIT COMPONENT API — Bidirectional communication with Streamlit host
// ═══════════════════════════════════════════════════════════════════════════════
let _userRole = 'viewer';
let _username = '';
let _receivedCloudState = false;

function _sendToStreamlit(type, data) {
    window.parent.postMessage(Object.assign({ isStreamlitMessage: true, type: type }, data), "*");
}

function _initStreamlit() {
    _sendToStreamlit("streamlit:componentReady", { apiVersion: 1 });
}

function _setFrameHeight(h) {
    _sendToStreamlit("streamlit:setFrameHeight", { height: h });
}

function saveToCloud() {
    // Send current state back to Streamlit Python for persistence
    _sendToStreamlit("streamlit:setComponentValue", { value: JSON.parse(JSON.stringify(state)) });
    const btn = document.getElementById('cloud-save-btn');
    if (btn) {
        btn.textContent = '✅ Guardado';
        btn.style.background = '#10b981';
        setTimeout(() => { btn.textContent = '☁️ Guardar en la Nube'; btn.style.background = ''; }, 2000);
    }
}
...
window.addEventListener("message", (event) => {
    ...
    _userRole = args.role || 'viewer';
    _username = args.username || '';

    // Load state from cloud if provided and non-empty
    if (args.state && Object.keys(args.state).length > 0 && !_receivedCloudState) {
        state = args.state;
        _receivedCloudState = true;
        console.log("State loaded from cloud.", Object.keys(state));
    }
    ...
    setTimeout(() => _setFrameHeight(document.body.scrollHeight), 200);
});

// Apply role-based UI restrictions
function applyRoleRestrictions() {
    const isAdmin = _userRole === 'admin';
    // Kitchen module — restrict recipe editing, menu editing for non-admins
    document.querySelectorAll('#dock-cocina').forEach(el => {
        // All users can view kitchen, but edit controls are restricted inside
    });
    ...
    const adminOnlySelectors = [
        ...
        '.dr-item-actions',
        '.table-actions',
        // Kitchen admin-only elements
        '#dish-form button[type="submit"]',
        '#delete-dish-btn',
        '#ai-gen-prep-btn',
        '#merge-section',
        '.import-section',
    ...
    // Disable drag for viewers (residents)
    if (!isAdmin) {
        document.querySelectorAll('.resident').forEach(el => {
            el.setAttribute('draggable', 'false');
            el.style.cursor = 'default';
        });
        // Disable delete buttons on residents
        document.querySelectorAll('.resident .icon-btn.danger').forEach(el => {
            el.style.display = 'none';
        });
    }
    // Update cloud save button visibility
    ...
    if (roleEl) {
        roleEl.textContent = isAdmin ? '👑 Admin' : '👀 Visor';
        roleEl.style.background = isAdmin ? '#f59e0b' : '#64748b';
    }
}
...
// State initialized from defaultState; cloud state overwrites this via Streamlit render event
let state = JSON.parse(JSON.stringify(defaultState));

// restoreData() — seed default residents if state is empty (first run)
...
function saveState() {
    // Fast local cache (survives page refresh within same browser)
